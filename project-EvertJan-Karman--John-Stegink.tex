...
The Hessian matrix is a matrix with which local extremes of a function kan be found. It helps to identify saddle points, local minima and local maxima. A saddle point (figure \ref{SaddlePoint}) is a function that does neither contain a local maximum or local minimum. From this point a function in some directions (using a certain set of variables) is a maximum and in some points a minimum. The type of extreme can be found by calculating the eigenvalues of the Hessian matrix.
\begin{itemize}
  \item It is a maximum when all eigenvalues are greater than 0.
  \item It is a maximum when all eigenvalues are less than 0.
  \item It is a saddlepoint when the product of all eigenvalues are 0.
  \item Otherwise it is indecisive.
\end{itemize}

The calculus and lineair algebra is not in the scope of this report. If you want to know why, \cite{nocedal2006numerical} is a good reference. 
...
2 & 12
\end{bmatrix}
\Rightarrow \lambda_1 = 9 +\srt{13}, \lambda_1 = 9 -\srt{13}. This is not a minimum
\]
...
\begin{figure}[htbp]
     \begin{minipage}{0.55\textwidth}
	    \includegraphics[width=7.5cm,keepaspectratio]{images/zadelpunt.png}
    	\caption{The red arrow denotes a saddle point}
    	\label{SaddlePoint}
	\end{minipage}
    \hfill
    \begin{minipage}{0.6\textwidth}
